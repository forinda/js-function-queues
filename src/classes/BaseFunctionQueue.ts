...
export class FunctionQueueManager {
  ...
  public generate_id(length = 16) {
    ...
    for (let i = 0; i < length; i++) {
      id += hex_characters[Math.floor(Math.random() * hex_characters.length)];
    }
    ...
    if (!this.is_empty()) {
      ...
      if (job) {
        ...
        const handleSuccess = <R>(data: R) => {
          ...
          console.info(
            `:${new Date().toLocaleString()}::LOG:: - (${job.function.name}) of id: ${
              job.id
          ...
          if (job.number_of_retries < job.retry_times) {
            ...
            console.error(
              `:${new Date().toLocaleString()}:LOG::(${
                job.number_of_retries
              })time(s):: number of retries for ${job.function.name}`
            );
